...
public abstract class SubArraySumX {
    ...
    public static long[] maxSumBoth(int[] numbers) {
        SubArraySumX[] problems = {contiguous(), nonContiguous()};

        for (int n : numbers)
            for (SubArraySumX p : problems)
                p.add(n);

        return new long[]{problems[0].max(), problems[1].max()};
    }

    public static SubArraySumX contiguous() {
        return new Contiguous();
    }

    public static SubArraySumX nonContiguous() {
    ...
    private static class Contiguous extends SubArraySumX {
    ...
    private static class NonContiguous extends SubArraySumX {
        private long sum;
